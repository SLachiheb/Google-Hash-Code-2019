...
private:
  ...
  IlcRevInt _boundary;
public:
  SequenceHeadSearch(IloCP cp,
                     IloIntervalSequenceVarArray sequences,
                     IloIntVar makespan);
  IloCP getCP() const { return _sequences.getCP();}
...
SequenceHeadSearch::SequenceHeadSearch(IloCP cp,
                                       IloIntervalSequenceVarArray sequences,
                                       IloIntVar makespan)
...
SequenceHeadSearch::complete(IloInt boundary) {
  ...
  _boundary.setValue(getCP(), boundary);
}
...
IlcGoal SequenceHeadSearch::makeGoal(IloCP cp) {
...
IlcGoal JSSPFLEXScheduler::initSearch(IloCP cp) {
