...
SetTimesSearch::SetTimesSearch(IloCP cp,
                               IloIntervalVarArray vars,
                               IloIntVar makespan,
                               ScheduleParam::Policy policy)
...
void SetTimesSearch::postpone(IlcInt index, IloInt date) {
  ...
  if ((!var.isAbsent() || (var.getStartMin() > date))) {
    _postponedDates[index].setValue(getCP(), date);
  }
}
...
IlcGoal SetTimesSearch::makeGoal(IloCP cp) {
...
IlcGoal RCPSPMMScheduler::initSearch(IloCP cp) {
