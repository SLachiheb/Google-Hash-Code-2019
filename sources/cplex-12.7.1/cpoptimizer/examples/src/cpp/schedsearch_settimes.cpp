...
public:
  SetTimesDominance(IloCP cp, RCPSPScheduler* scheduler);
  IloCP getCP() const { return _intervals.getCP();}
  ...
  IlcGoal makeGoal(IloCP cp);
};

SetTimesDominance::SetTimesDominance(IloCP cp, RCPSPScheduler* s) 
...
void SetTimesDominance::postpone(IloInt i, IloInt date) {
  if (!(_intervals[i].isAbsent() ||
    ...
    _postponeDates[i].setValue(getCP(), date);
}
...
IlcGoal SetTimesDominance::makeGoal(IloCP cp) {
...
public:
  setTimesResources(IloCP cp, RCPSPScheduler* scheduler);
  IloCP getCP() const { return _intervals.getCP();}
  IloBool select(IloInt &p, IloInt& date);
  void postpone(IloInt p, IloInt date);
  void schedule(IloInt p, IloInt date) {
...
setTimesResources::setTimesResources(IloCP cp, RCPSPScheduler* s) 
  :_intervals(IlcIntervalVarArray
              (cp, s->getJobs()->getIntervals().getSize()))
...
IlcGoal setTimesResources::makeGoal(IloCP cp) {
...
IlcGoal RCPSPScheduler::initSearch(IloCP cp) {
  switch(_heuristics) {
  case 1:
