...
void MakeConveyorBelt(IloModel model,
  ...
  for(IloInt i = 0; i < nb; ++nb) {
    ...
    load.setOptional();
    sprintf(buffer, "Load<Item%ld>", i);
    load.setName(buffer);
    loads.add(load);

    IloIntervalVar unload(env, unloadTime);
    unload.setOptional();
    sprintf(buffer, "Unload<Item%ld>", i);
  ...
  try {
    ...
    for (IloInt i = 0; i < nbJobs; i++) {
      ...
      for (IloInt j = 0; j < nbMachines; j++) {
        ...
        sprintf(name, "Job%ldShop%ld", i, j);
        ti.setName(name);
        machines[j].add(ti);
        if (0 != prec.getImpl())
