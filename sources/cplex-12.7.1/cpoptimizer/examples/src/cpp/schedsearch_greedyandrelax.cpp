...
void MakeJob(IloModel model,
  ...
  top.setOptional();
  sprintf(buffer, "Job%ld<Type%ld>", oindex, type);
  top.setName(buffer);
  IloInt i, w, j;
  ...
  for (i = 0 ; i < numberOfWorkers; i++) {
    ...
    w = job->getWorkers(i);    
    sprintf(buffer, "Job%ld<Type%ldWorker%ld>", oindex, type, w);
    span.setName(buffer);
    jobs.add((IloAny) job);
...
public:
  JobRelaxation(IloCP cp, Job* job);
...
JobRelaxation::JobRelaxation(IloCP cp, Job* job) 
  :_job(job)
  ,_relaxedLevel(0)
...
private:
  ...
  IloInt _guilty;
public:
  RelaxGreedySearch(IloCP cp, IloAnyArray jobs);
  void constrain();
...
public:
  JobConstraint(IloCP cp, RelaxGreedySearch* s)
    :IlcConstraintI(cp)
    ,_s(s)
  {}
  virtual void post() {};
...
RelaxGreedySearch::RelaxGreedySearch(IloCP cp, IloAnyArray jobs)
...
IlcGoal RelaxGreedySearch::makeGoal(IloCP cp) {
...
IlcGoal RelaxGreedyScheduler::initSearch(IloCP cp) {
