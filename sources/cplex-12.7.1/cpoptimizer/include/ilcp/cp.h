...
#ifndef ILC_DONT_USE_XMLCONTEXT
# define ILC_USE_XMLCONTEXT
# define ILC_USE_TOBRANCH
#endif
...
private:
  ...
  // now defined in ilo-search.template
  IloBool _next() const;
...
public:
  ...
  ///////////////////////////////////////////////////////////////////////////
   ...
   enum IntParam {   
    ...
    MasterFiltersSolutions = 117,   
    
    
    WorkerFiltersSolutions = 118,   
...
void extract (const IloModel model) const;
 ...
 void end();
  ...
  ILCDEPRECATED IloBool solve(const IloSearchPhaseArray phaseArray) const 
#endif
  {
    IlcCPOAssert(_impl != 0, "IloCP: empty handle");
    IlcCPOAssert(phaseArray.getImpl() != 0, "IloSearchPhaseArray: empty handle");
  ...
  ILCDEPRECATED IloBool solve(const IloSearchPhase phase) const 
#endif
  {
    IlcCPOAssert(_impl != 0, "IloCP: empty handle");
    IlcCPOAssert(phase.getImpl() != 0, "IloSearchPhase: empty handle");
  ...
  ILCDEPRECATED void startNewSearch(const IloSearchPhaseArray phaseArray) const 
#endif
  {
    IlcCPOAssert(_impl != 0, "IloCP: empty handle");
    IlcCPOAssert(phaseArray.getImpl() != 0, "IloSearchPhaseArray: empty handle");
  ...
  ILCDEPRECATED void startNewSearch(const IloSearchPhase phase) const
#endif
  {
    IlcCPOAssert(_impl != 0, "IloCP: empty handle");
    IlcCPOAssert(phase.getImpl() != 0, "IloSearchPhase: empty handle");
...
public:
  ...
  static FailureStatus convertFailureStatus(IloInt f)  {
    switch (f) {
  ...
  static IloCP::ParameterValues filterToInferenceLevel(IlcFilterLevel fl) {
    switch (fl) {
  ...
  static IloCP::IntParam filterToInferenceCt(IlcFilterLevelConstraint fl) {
    switch (fl) {
...
typedef IloSolver::FailureStatus        IlcSolverFailureStatus;
...
public:
  ...
  IloNum getValue(IloCP cp, IloNumVar var) const;
  IloInt getValue(IloCP cp, IloIntVar var) const;
...
# define ILCRTTIDECL                                                    \
        static IlcTypeIndex _rttiIndex;                                       \
public:                                                                 \
                                                                           \
...
#define ILCRTTIN(_thisMacro, _this, _baseMacro, _base, _templateMacro, _template) \
        ...
        _templateMacro _template \
        IlcTypeIndex _thisMacro _this::_rttiIndex = 0; \
        _templateMacro _template \
