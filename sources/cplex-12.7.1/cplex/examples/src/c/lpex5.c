...
main (void)
{
   ...
   status = CPXaddfpdest (env, cpxresults, fpout);
   if ( status ) {
...
TERMINATE:
   ...
   if ( fpout != NULL ) {
      int chanstat;
      chanstat = CPXdelfpdest (env, cpxresults, fpout);
      if ( chanstat ) {
