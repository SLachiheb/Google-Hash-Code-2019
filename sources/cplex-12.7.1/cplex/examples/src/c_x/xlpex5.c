...
main (void)
{
   ...
   fpout = CPXXfopen ("xlpex5.msg", "w");
   if ( fpout == NULL ) {
      CPXXmsg (cpxerror, "Failed to open xlpex5.msg file for output.\n");
      goto TERMINATE;
   }
   status = CPXXaddfpdest (env, cpxresults, fpout);
   if ( status ) {
   ...
   CPXXmsg (cpxresults, "\nSolution status = %d\n", solstat);
   CPXXmsg (cpxresults, "Solution value  = %f\n\n", objval);
   ...
   for (i = 0; i < cur_numrows; i++) {
      CPXXmsg (cpxresults, "Row %d:  Slack = %10f  Pi = %10f\n", 
               i, slack[i], pi[i]);
   }

   for (j = 0; j < cur_numcols; j++) {
      CPXXmsg (cpxresults, "Column %d:  Value = %10f  Reduced cost = %10f\n",
               j, x[j], dj[j]);
   }
...
TERMINATE:
   ...
   if ( fpout != NULL ) {
      int chanstat;
      chanstat = CPXXdelfpdest (env, cpxresults, fpout);
      if ( chanstat ) {
