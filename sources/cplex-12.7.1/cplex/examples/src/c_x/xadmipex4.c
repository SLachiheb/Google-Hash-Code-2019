...
/* Examples xadmipex4.c and xadmipex5.c both solve the MIPLIB
   3.0 model noswot.mps by adding user cuts.  xadmipex4.c adds
   these cuts to the cut table before the branch-and-cut
   process begins, while xadmipex5.c adds them through the
   user cut callback during the branch-and-cut process. 
   
   This example illustrates use of cplexx.h
             ...
             CPXLPptr  lp)
{
   ...
   /* Alternative approach:  Replace CPXXaddusercuts() by
      the following CPXXaddrows() call, and delete the call to 
      CPXXsetintparam (env, CPXPARAM_Preprocessing_Linear, 0) in main().
   ...
   status = CPXXaddrows (env, lp, 0, cuts, nz, cutrhs, 
